...
import { Container, Box, Typography, Button, Grid, Card, CardContent, Skeleton, Chip } from "@mui/material";
  ...
  return (
    <Box sx={{ minHeight: "100vh", bgcolor: "background.default", color: "text.primary", p: 4 }}>
      <Container maxWidth="lg" sx={{ textAlign: "left" }}>
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: 1, borderColor: "divider", pb: 2, mb: 4 }}>
          <Box>
          ...
          <Grid container spacing={3}>
            <Grid size={{ xs: 12, md: 8 }}>
              ...
              {loadingMarkets ? (
                ...
                <Box sx={{ p: 2, borderRadius: 1, border: "1px dashed", borderColor: "divider", color: "text.secondary" }}>
                  ...
                  {markets.map((market: Market) => (
                    <Card key={market.id} variant="outlined">
                      <CardContent sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", "&:last-child": { pb: 2 } }}>
                        <Box>
                          <Typography variant="caption" fontFamily="monospace" color="text.secondary" display="block" gutterBottom>
                            {market.id}
                          </Typography>
                          <Typography variant="h6" component="h3" lineHeight={1.2}>
                            {market.question}
                          </Typography>
                        </Box>
                        <Box sx={{ textAlign: "right", pl: 2 }}>
                          <Typography
                            variant="h5"
                            fontWeight="bold"
                            color={(market.confidenceScore ?? 0) > 0.8 ? "success.main" : "info.main"}
                          >
            ...
            <Grid size={{ xs: 12, md: 4 }}>
              ...
              <Box sx={{ mt: 4, p: 2, bgcolor: "action.hover", borderRadius: 1 }}>
                <Typography variant="subtitle2" component="h3" color="text.secondary" textTransform="uppercase" gutterBottom>
                  Live News Feed
                </Typography>
                ...
                <Box sx={{ maxHeight: 256, overflowY: "auto", pr: 1, display: "flex", flexDirection: "column", gap: 1 }}>
                  {newsFeed.length === 0 ? (
